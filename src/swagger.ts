/**
 * OpenAPI (Swagger) specification for the Karma App API.
 *
 * Served as interactive docs at `/api-docs` (wired up in `server.ts`). The path
 * keys are assembled from the shared `Paths` constants so the documentation
 * stays in sync with the routes registered in `routes/api.ts`.
 *
 * The spec is a plain object (no comment scraping) so it behaves identically in
 * development and in the production build, which compiles from `src/` to `dist/`
...
// Join path segments and turn Express params (`:id`) into OpenAPI ones (`{id}`).
const route = (...segments: string[]): string =>
  segments.join('').replace(/:([A-Za-z0-9_]+)/g, '{$1}');

// Reference a reusable schema defined under `components.schemas`.
const ref = (name: string) => ({ $ref: `#/components/schemas/${name}` });

// Shorthand for a JSON response/request body of the given schema.
...
const swaggerSpec = {
  ...
  info: {
    ...
    version: '1.0.0',
    description:
      'REST API for the Karma app: authentication, tasks, file uploads, video ' +
  ...
  paths: {
    ...
    [route(B, K.Base, K.UploadFile)]: {
      post: {
        ...
        description:
          'Stores the uploaded file and marks the task status as `Pending`.',
        security: bearer,
        requestBody: {
          required: true,
          content: {
    ...
    [route(B, K.Base, K.ValidateTaskStatus)]: {
      post: {
        ...
        description:
          'Reviews a submission. On success a push notification is sent to the ' +
          'task owner.',
        security: bearer,
        parameters: [
          {
    ...
    [route(B, K.Base, K.Stream)]: {
      get: {
        ...
        description:
          'Uses the temporary id returned by the video URL endpoint. No auth ' +
    ...
    // ** User Management (example CRUD routes) ** //
